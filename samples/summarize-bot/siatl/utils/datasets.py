...
def read_corpus_sum(sentences, posts, labels, tokenize, vocab=None, vocab_size=None):
    if vocab is not None:
        _vocab = vocab
    else:
        _vocab = Vocab()
    ...
    input_ = []
    posts_ = []
    for line1, line2, y in zip(sentences, posts, labels):
    ...
    return _vocab, _data, _data_2, labels_, input_, posts_
...
#         import random
#         list_idx = [index for index, x in enumerate(self.lengths)]
#         random.shuffle(list_idx)
#         return iter(numpy.array(list_idx))
...
class SUMDataset(Dataset):
    def __init__(self, input, posts,  labels=None, seq_len=0, preprocess=None,
        ...
        self.vocab, self.data, self.data2, self.labels, self.input, self.posts = read_corpus_sum(input, posts, labels, self.preprocess, vocab, vocab_size)
